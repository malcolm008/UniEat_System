...
const getUsers = async (req, res, next) => {
    try {
        ...
        return pagination(res, rows.rows, parseInt(cnt.rows[0].count), page, limit);
    } catch (err) { next(err); }
};
...
const createUser = async (req, res, next) => {
    try {
        ...
        return created(res, rows[0], 'Staff member created successfully');
    } catch (err) { next(err); }
};
...
const deleteUser = async (req, res, next) => {
    try {
        ...
        if (checkUser.rows[0].role === 'admin') {
            return error(res, 'Cannot delete admin users from this endpoint', 403);
        }
